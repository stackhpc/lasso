/* ----------------------------------------------------------------------------
 * This file was automatically generated by SWIG (http://www.swig.org).
 * Version 1.3.24
 *
 * Do not make changes to this file unless you know what you are doing--modify
 * the SWIG interface file instead.
 * ----------------------------------------------------------------------------- */

package com.entrouvert.lasso;

public class NameRegistration {
  private long swigCPtr;
  protected boolean swigCMemOwn;

  protected NameRegistration(long cPtr, boolean cMemoryOwn) {
    swigCMemOwn = cMemoryOwn;
    swigCPtr = cPtr;
  }

  protected static long getCPtr(NameRegistration obj) {
    return (obj == null) ? 0 : obj.swigCPtr;
  }

  protected NameRegistration() {
    this(0, false);
  }

  protected void finalize() {
    delete();
  }

  public void delete() {
    if(swigCPtr != 0 && swigCMemOwn) {
      swigCMemOwn = false;
      lassoJNI.delete_NameRegistration(swigCPtr);
    }
    swigCPtr = 0;
  }

  public void setIdentity(Identity identity) {
    lassoJNI.set_NameRegistration_identity(swigCPtr, Identity.getCPtr(identity));
  }

  public Identity getIdentity() {
    long cPtr = lassoJNI.get_NameRegistration_identity(swigCPtr);
    return (cPtr == 0) ? null : new Identity(cPtr, false);
  }

  public boolean getIsIdentityDirty() {
    return lassoJNI.get_NameRegistration_isIdentityDirty(swigCPtr);
  }

  public boolean getIsSessionDirty() {
    return lassoJNI.get_NameRegistration_isSessionDirty(swigCPtr);
  }

  public String getMsgBody() {
    return lassoJNI.get_NameRegistration_msgBody(swigCPtr);
  }

  public String getMsgRelayState() {
    return lassoJNI.get_NameRegistration_msgRelayState(swigCPtr);
  }

  public String getMsgUrl() {
    return lassoJNI.get_NameRegistration_msgUrl(swigCPtr);
  }

  public void setNameIdentifier(SamlNameIdentifier nameIdentifier) {
    lassoJNI.set_NameRegistration_nameIdentifier(swigCPtr, SamlNameIdentifier.getCPtr(nameIdentifier));
  }

  public SamlNameIdentifier getNameIdentifier() {
    long cPtr = lassoJNI.get_NameRegistration_nameIdentifier(swigCPtr);
    return (cPtr == 0) ? null : new SamlNameIdentifier(cPtr, false);
  }

  public void setRemoteProviderId(String remoteProviderId) {
    lassoJNI.set_NameRegistration_remoteProviderId(swigCPtr, remoteProviderId);
  }

  public String getRemoteProviderId() {
    return lassoJNI.get_NameRegistration_remoteProviderId(swigCPtr);
  }

  public void setRequest(SamlpRequestAbstract request) {
    lassoJNI.set_NameRegistration_request(swigCPtr, SamlpRequestAbstract.getCPtr(request));
  }

  public SamlpRequestAbstract getRequest() {
	long cPtr = lassoJNI.get_NameRegistration_request(swigCPtr);
	return (cPtr == 0) ? null : (SamlpRequestAbstract) lassoJNI.downcast_node(cPtr);
}

  public void setResponse(SamlpResponseAbstract response) {
    lassoJNI.set_NameRegistration_response(swigCPtr, SamlpResponseAbstract.getCPtr(response));
  }

  public SamlpResponseAbstract getResponse() {
	long cPtr = lassoJNI.get_NameRegistration_response(swigCPtr);
	return (cPtr == 0) ? null : (SamlpResponseAbstract) lassoJNI.downcast_node(cPtr);
}

  public void setServer(Server server) {
    lassoJNI.set_NameRegistration_server(swigCPtr, Server.getCPtr(server));
  }

  public Server getServer() {
    long cPtr = lassoJNI.get_NameRegistration_server(swigCPtr);
    return (cPtr == 0) ? null : new Server(cPtr, false);
  }

  public void setSession(Session session) {
    lassoJNI.set_NameRegistration_session(swigCPtr, Session.getCPtr(session));
  }

  public Session getSession() {
    long cPtr = lassoJNI.get_NameRegistration_session(swigCPtr);
    return (cPtr == 0) ? null : new Session(cPtr, false);
  }

  public void setOldNameIdentifier(SamlNameIdentifier oldNameIdentifier) {
    lassoJNI.set_NameRegistration_oldNameIdentifier(swigCPtr, SamlNameIdentifier.getCPtr(oldNameIdentifier));
  }

  public SamlNameIdentifier getOldNameIdentifier() {
    long cPtr = lassoJNI.get_NameRegistration_oldNameIdentifier(swigCPtr);
    return (cPtr == 0) ? null : new SamlNameIdentifier(cPtr, false);
  }

  public NameRegistration(Server server) {
    this(lassoJNI.new_NameRegistration(Server.getCPtr(server)), true);
  }

  public static NameRegistration newFromDump(Server server, String dump) {
    long cPtr = lassoJNI.NameRegistration_newFromDump(Server.getCPtr(server), dump);
    return (cPtr == 0) ? null : new NameRegistration(cPtr, true);
  }

  public int setIdentityFromDump(String dump) {
    return lassoJNI.NameRegistration_setIdentityFromDump(swigCPtr, dump);
  }

  public int setSessionFromDump(String dump) {
    return lassoJNI.NameRegistration_setSessionFromDump(swigCPtr, dump);
  }

  public int buildRequestMsg() {
    return lassoJNI.NameRegistration_buildRequestMsg(swigCPtr);
  }

  public int buildResponseMsg() {
    return lassoJNI.NameRegistration_buildResponseMsg(swigCPtr);
  }

  public String dump() {
    return lassoJNI.NameRegistration_dump(swigCPtr);
  }

  public int initRequest(String remoteProviderId, int httpMethod) {
    return lassoJNI.NameRegistration_initRequest(swigCPtr, remoteProviderId, httpMethod);
  }

  public int processRequestMsg(String requestMsg) {
    return lassoJNI.NameRegistration_processRequestMsg(swigCPtr, requestMsg);
  }

  public int processResponseMsg(String responseMsg) {
    return lassoJNI.NameRegistration_processResponseMsg(swigCPtr, responseMsg);
  }

  public int validateRequest() {
    return lassoJNI.NameRegistration_validateRequest(swigCPtr);
  }

}
